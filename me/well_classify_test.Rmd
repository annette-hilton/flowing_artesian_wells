...
me_mod_fromarc <- readr::read_csv(here::here("data", "me", "me_modern_toclassify.txt")) %>% 
  clean_names()
...
Confined aquifers, including confining units except the Vicksburg (first confining unit) 
...
# unconfined = mrva, vkbg
# confined = ucaq, mccu, mcaq, lccu, lcaq, mwaq, lwaq, mdwy 

me_modern_check <- me_class_mod %>% 
  rowwise() %>% 
  mutate(check = 
           ifelse(aquifer %in% c("ucaq", "mccu", "mcaq", "lccu", "lcaq", "mwaq", "lwaq", "mdwy") & 
                    code %in% c("Confined single aquifer", "Confined multiple aquifers"), "true_c", 
           ifelse(aquifer %in% c("ucaq", "mccu", "mcaq", "lccu", "lcaq", "mwaq", "lwaq", "mdwy") & 
                    is.na(code), "no_data_c", 
           ifelse(aquifer %in% c("ucaq", "mccu", "mcaq", "lccu", "lcaq", "mwaq", "lwaq", "mdwy") & 
                    code %in% c("Unconfined single aquifer", "Unconfined multiple aquifer"), "wrong_c", 
           ifelse(aquifer %in% c("mrva", "vkbg") & is.na(code), "no_data_uc",
           ifelse(aquifer %in% c("mrva", "vkbg") & code %in% c("Unconfined single aquifer", "Unconfined multiple aquifer"), "true_uc", 
           ifelse(aquifer %in% c("mrva", "vkbg") & code %in% c("Confined single aquifer", "Confined multiple aquifers"), "wrong_uc", 
                                                  NA)))))))
...
# Check wrong confined wells 
second_check <- me_modern_check %>% 
  filter(check == "wrong_c") %>% 
  rowwise() %>% 
  mutate(second_check = 
           ifelse(aquifer == "ucaq" & is.na(vkbg_surf), "understandable",
           ifelse(aquifer == "mccu" & is.na(vkbg_surf) & is.na(ucaq_surf), "understandable",
           ifelse(aquifer == "mcaq" & is.na(vkbg_surf) & is.na(ucaq_surf) & is.na(mccu_surf), "understandable", 
           ifelse(aquifer == "lccu" & is.na(vkbg_surf) & is.na(ucaq_surf) & is.na(mccu_surf) & is.na(mcaq_surf), "understandable", 
           ifelse(aquifer == "lcaq" & is.na(vkbg_surf) & is.na(ucaq_surf) & is.na(mccu_surf) & is.na(mcaq_surf) 
                  & is.na(lccu_surf), "understandable", 
           ifelse(aquifer == "mwaq" & is.na(vkbg_surf) & is.na(ucaq_surf) & is.na(mccu_surf) & is.na(mcaq_surf) 
                  & is.na(lccu_surf) & is.na(lcaq_surf), "understandable", 
           ifelse(aquifer == "lwaq" & is.na(vkbg_surf) & is.na(ucaq_surf) & is.na(mccu_surf) & is.na(mcaq_surf) 
                  & is.na(lccu_surf) & is.na(lcaq_surf) & is.na(mwaq_surf), "understandable", 
           ifelse(aquifer == "mdwy" & is.na(vkbg_surf) & is.na(ucaq_surf) & is.na(mccu_surf) & is.na(mcaq_surf) 
                  & is.na(lccu_surf) & is.na(lcaq_surf) & is.na(mwaq_surf) &is.na(lwaq_surf), "understandable",
                  "bad")))))))))



summary_secondcheck_2 <- second_check %>% 
  group_by(second_check) %>% 
  count(second_check)

bad <- second_check %>% 
  filter(second_check == "bad")
...
# Modern data 
me_confined_mod <- me_class_mod %>% 
  filter(!aquifer %in% c("mrva", "vkbg")) %>% 
  mutate(artesian = 
           ifelse(median < 0, 1, 0))
...
# 1900 data 
me_confined_1900 <- me_class_1900 %>% 
  filter(!aquifer %in% c("mrva", "vkbg")) 
