...
```{r}
# CA drilling wells
# Source: see `meta_data_read_me.Rmd` 

# Load raw data 
na_values <- c("-9999.0000000")

ca_drill_raw <- readr::read_csv(here::here("data", "cv", "ca_wells_elv.txt"), na = na_values) 

# Initial tidy
ca_drill_tidy <- ca_drill_raw %>% 
  clean_names() %>% 
  filter(!total_comp == 0) %>% 
  filter(year >= 2010) %>% 
  select(wcr_number, decimal_la, decimal_lo, total_comp, static_wat, date_work, year, cali) 

# Make consistent with USGS data to join them 
ca_drill_tidy_final <- ca_drill_tidy %>% 
  mutate(id_x = "ca", 
         artesian =  
           ifelse(static_wat <= 0, 1, 0)) %>% 
  mutate(national_aquifer_x = NA, 
         local_aquifer_x = NA, 
         code = NA,
         depth_m_x = NA) %>% 
  rename(site_no = wcr_number, 
         dec_lat_va_x = decimal_la, 
         dec_long_va_x = decimal_lo, 
         well_depth_va = total_comp, 
         median = static_wat, 
         level_date = date_work,
         level_year = year, 
         rastervalu = cali) %>% 
...
cv_modern_final <- rbind(ca_drill_tidy_final, cv_modern)
