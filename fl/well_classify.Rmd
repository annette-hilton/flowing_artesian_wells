...
fl_analysis_first <- arc_join_fl_tidy %>% 
...
fl_analysis <- fl_analysis_first %>% 
  mutate(ucu_top = uf_top + ucu_thick, 
         surf_top = ucu_top + surf_thick) %>% 
  select(site_no, dec_lat_va, dec_long_va, well_depth_va, 
         hole_depth_va, alt_va, topo_ft, surf_top, ucu_top,
         uf_top, buca_top, buca_bot, ocaplpz_top, appz_top, lisapcu_top, mapcu_top, lf_top, 
        lappz_top, glauc_top, oldspz_top, oldspz_bot, well_bottom)

write_tsv(fl_analysis, here::here("fl_data", "fl_analysis.txt"))
```
...
write_tsv(summary_fl, here::here("fl_data", "summary_fl.txt"))
```

#### Confined and Unconfined Well Criteria 

Unconfined aquifer (SURF): 

- Criteria: Must have at least 10 m (33 ft) of confining material below 
- Code: If the aquifer is SURF, then calculate the sum of the confining unit thicknesses (UCU, Bucatunna). If the sum of the confining unit thicknesses is >= 33 ft (10m), then "keep" the well. 
...
```{r}
# SURF
surf <- fl_cum_thickness %>% 
  filter(aquifer == "surf" & keep == "keep")

# Add character value in front of site_no column as workaround for ArcGIS issue

surf$site_no <- sub("^", "x", surf$site_no)

# Simplify for ArcGIS 

surf_simple <- surf %>% 
  select(site_no, dec_lat_va, dec_long_va, aquifer)

# Write as tsv 
write_tsv(surf_simple, here::here("fl_data", "arc_input", "surf.txt"))
...
fl_confined <- fl_cum_thickness %>% 
  filter(keep == "keep" & !aquifer == "surf")

# Add character value in front of site_no column as workaround for ArcGIS issue

fl_confined$site_no <- sub("^", "x", fl_confined$site_no)

# Simplify for ArcGIS 

fl_confined_simple <- fl_confined %>% 
  select(site_no, dec_lat_va, dec_long_va, aquifer, ucu_thick)

# Write as tsv 
write_tsv(fl_confined_simple, here::here("fl_data", "arc_input", "fl_confined.txt"))
```
