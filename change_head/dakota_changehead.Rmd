...
early1900s_sd <- read_csv(here::here("change_head", "data", "1900s_sd.txt"))

# Modern 2010-2022 clean file 

modern_sd <- read_csv(here::here("change_head", "data", "modern_sd.txt"))
```

##### Aquifer units to analyze in Dakota: 

- Confined Dakota aquifer 
...
sd_mod_clean$artesian <- as.character(sd_mod_clean$artesian)
...
sd_1900s_clean$artesian <- as.character(sd_1900s_clean$artesian)
```
...
sd <- ggplot() + 
  geom_point(data = sd_1900s_clean, aes(x = x_axis, y = head_m, color = artesian), size = 2.5) +
  geom_line(data = sd_1900s_clean, aes(x = x_axis, y = head_m)) +
  geom_point(data = sd_mod_clean, aes(x = x_axis, y = head_m, color = artesian), shape = 15, size = 2.5) +
  geom_line(data = sd_mod_clean, aes(x = x_axis, y = head_m)) +
  theme_minimal() +
  labs(x = "", 
...
ggsave(plot = sd, "sd.eps", path = "eps_files", width = 10, height = 5, units = "in")
...
# 1900s 
sd_1900_summary <- sd_1900s_clean %>%  
  dplyr::summarise(
    mean = mean(head_m), 
...
kable(sd_1900_summary, caption = "Table 1. Summary statistics for Pre-1910")

kable(sd_mod_summary, caption = "Table 1. Summary statistics for Post-2010")
