...
early1900s_dem_tidy <- early1900s_dem %>% 
  ...
  filter(!is.na(ned10m_bilinear_cusa_albers102003))

# Join with metadata file/full 1900s file 

early1900s_final <- early1900s_dem_tidy %>% 
  full_join(early1900s_arc, by = "id") 


duplicated(early1900s_arc)
sum(duplicated(early1900s_arc))

duplicated(early1900s_dem_tidy)
sum(duplicated(early1900s_dem_tidy))

test_distinct <- early1900s_dem_tidy %>% 
  distinct(id, .keep_all = TRUE)

test_distinct_2 <- early1900s_arc %>% 
  distinct(id, .keep_all = TRUE)
...
test <- test_distinct_2 %>% 
  dplyr::inner_join(test_distinct, by = "id")

test_left <- test_distinct_2 %>% 
  dplyr::left_join(test_distinct, by = "id")

test_right <- test_distinct_2 %>% 
  dplyr::right_join(test_distinct, by = "id")
```
