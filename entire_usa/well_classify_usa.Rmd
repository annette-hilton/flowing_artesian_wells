...
1. Spatial join modern data and historical data (`modern_final.txt` from `modern_data_cleaning.Rmd`; `early1900s_final.txt` from `early1900s_cleaning.Rmd`) with aquifer file and depth data: processsed in ArcPro (file name:`2022_05_16_AquifersWithDepthConfined`) 
...
modern_depths <- readr::read_csv(here::here("data", "depths", "modern_depths_063022.txt"))

# 1900s data 
early1900s_depths <- readr::read_csv(here::here("data", "depths", "historical_depths_062922.txt"))
...
historical_analysis_depth <- early1900s_depths %>% 
  mutate(depth_m = well_depth_ft * 0.3048) %>% 
  mutate(confined_depth = ifelse(depth_m >= DepthConfi, "confined depth", NA))
...
write_tsv(both_hist_modern, here::here("output_data", "depths", "both_aquiferstimeperiod.txt"))
```
...
write_tsv(joined, here::here("output_data", "depths", "joined_fractions_062922.txt"))
```
