...
```{r}
# old CA drilling wells? 

ca_drill <- readr::read_csv(here::here("data", "cv", "ca_wells_elv.txt" )) 

ca_drill_tidy <- ca_drill %>% 
  clean_names() %>% 
  filter(!total_comp == 0) %>% 
  filter(year >= 2010) %>% 
  select(wcr_number, decimal_la, decimal_lo, total_comp, static_wat, year, cali)

write_tsv(ca_drill_tidy, here::here("output_data", "cv", "ca_drill_forarc.txt"))
```

## Data Analysis (Pre-Arc)

### Refine dataset for the CV  
